...
```python
r = pd.concat(datas, axis=0, join='outer', ignore_index=False, keys=['x', 'y', 'z'])
```
...
*Student*        id      name    age     gender    **cid**

*Classroom*    cid    cname   cloc
...
select 
	...
	c.cname,
	c.cloc
from Student s join Classroom c 
	on s.cid=c.cid
where s.age>14 and xxx and  xxx 
```
